...
> For tool setup and MCP configuration, see [MCP-SETUP.md](../MCP-SETUP.md). Commands work without any connected tools — provide context manually if needed.
...
- **your knowledge base** (Notion, Confluence, etc.) via MCP (if connected): Search for research documents, interview notes, survey results
- **your feedback tool** (Intercom, Canny, etc.) via MCP (if connected): Pull recent support tickets, feature requests, bug reports
- **your analytics tool** (Amplitude, Pendo, etc.) via MCP (if connected): Pull usage data, funnel metrics, behavioral data
- **your meeting notes tool** (Fireflies, Gong, etc.) via MCP (if connected): Pull interview recordings, meeting summaries, and discussion notes
