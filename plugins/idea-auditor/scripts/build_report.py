...
def score_bar(score: float | None, max_score: float = 5.0) -> str:
    """Simple text bar representation of score_bruto."""
    if score is None:
        return "—"
...
def build_report(scorecard: dict, evidence: dict | None) -> str:
    ...
    lines.append(f"")
    lines.append(f"---")
    lines.append(f"_Report generated by `build_report.py`. Source of truth: `REPORTS/scorecard-{scored_at}.json`._")
...
def main() -> None:
    ...
    report_md = build_report(scorecard, evidence)
