...
usage() {
  ...
  echo "  --strict    Also check for presence of 9 Gate B mandatory artefacts in the same directory"
  echo ""
  echo "Checks:"
  echo "  - RTM file exists and is non-empty"
  echo "  - RTM table has required columns (REQ-ID, AC-ID, Status)"
  echo "  - REQ-ID count (reports count)"
  echo "  - AC-ID count (reports count)"
  echo "  - Ratio check: warns if AC count < REQ count (possible orphaned requirements)"
  echo "  - In strict mode: checks that 9 Gate B artefacts exist alongside the RTM"
  echo ""
  echo "Exit codes:"
